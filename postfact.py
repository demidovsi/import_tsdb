                                    ...
                                    if "value_string" in js[i]:
                                        ...
                                        if mt != '':
                                            ...
                                            if not work or (work == ""):  # можно запускать поток
                                                at = TPostOne(typeobj_code, param_code, t_beg, t_end)
                                                at.start()
...
class TPostOne(threading.Thread):
    ...
    def __init__(self, typeobj_code, param_code, t_beg, t_end):
        threading.Thread.__init__(self)
        self.daemon = True
    ...
    def run(self):
        for mas in commondata.mas_js:
            try:
                if (mas['typeobj_code'] == self.typeobj_code) and (mas['param_code'] == self.param_code):
                    ...
                    while t0 <= t1:
                        ...
                        txt, result = commondata.send_rest(
                            ...
                            '&value=' + time.strftime('t_beg=%Y-%m-%d %H:%M:%S', time.localtime(self.dt_beg)) +
...
def work_one_time(tek_time, discret, equipment_id, parameter_id, typeobj_code, param_code, id):
    ...
    dt = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(tek_time))
    toper = time.time()
